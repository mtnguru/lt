/*
  ...
  An Arduino-Stream that wraps an instance of MqttClient reconnecting to
...
{
  public:
    EthernetClientStream(MqttClient &client, IPAddress localip, IPAddress ip, const char* host, uint16_t port);
    int available();
    int read();
    int peek();
    void flush();
 ...
 */
EthernetClientStream::EthernetClientStream(MqttClient &client, IPAddress localip, IPAddress ip, const char* host, uint16_t port)
  : client(client),
    localip(localip),
    ip(ip),
    host(host),
