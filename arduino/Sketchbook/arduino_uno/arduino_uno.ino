/*
  *  Arduino Uno with Ethernet - arduino.ino
*/
...
/**
 * initInternet - Initialize the wifi, get IP
 */
void initInternet() {
...
void requestConfig() {
  ...
  snprintf(payload,payloadSize,"{\"cmd\":\"requestConfig\",\"ip\":\"%s\"}", lip );
...
/*
        char svalue[20];
        if (value > 500) {
          snprintf(out, outSize, "temperature out of range: %s", dtostrf(value,2,2,&svalue[strlen(svalue)]));
          logit(0,ME,f,out, NULL);
        } else {
          snprintf(out, outSize, "value=%s", dtostrf(value,2,2,&svalue[strlen(svalue)]));
          logit(1,MD,f,"temperature acquired", out);

          snprintf(out, outSize, "%s value=%s", input->tags, dtostrf(value,2,2,&svalue[strlen(svalue)]));
...
void setup() {
  ...
  snprintf(mqttRspSub, topicSize, "a/admin/rsp/%s", lip);   // admin responses
  res = mqttClient.subscribe(mqttRspSub);
...
/*
  if (ethernetClient.status() != WL_CONNECTED) {
    logit(0,ME,f,"Internet not connected - reset the arduino",NULL);
    delay(500);
    resetFunction();
  }
*/
